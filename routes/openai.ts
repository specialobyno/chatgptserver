...
router.post("/text", async (req, res) => {
  try {
    ...
    if (!message) throw new Error("No message response")

    await axios.post(`https://api.chatengine.io/chats/${activeChatId}/messages/`, { text: message.content }, headers)
...
router.post("/code", async (req, res) => {
  try {
    ...
    const message = data.choices[0].message
    if (!message) throw new Error("No message response")
    await axios.post(`https://api.chatengine.io/chats/${activeChatId}/messages/`, { text: `<code>${message.content}</code>` }, headers)
